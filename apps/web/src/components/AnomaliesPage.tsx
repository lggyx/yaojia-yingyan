    ...
    <aside className="rounded-md border border-sentinel-line bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 grid gap-3">
        ...
        <StepCard title="4. 研判结论" active={Boolean(aiReport)}>
          ...
          {aiReport ? (
            <>
              {aiReport.investigation?.usedMock === false ? (
                <span className="mb-2 inline-flex items-center gap-1 rounded-full bg-[#d1f0e1] px-2 py-0.5 font-mono text-[10px] font-semibold uppercase tracking-[0.08em] text-[#1a5c3a]">
                  <span className="h-1.5 w-1.5 rounded-full bg-[#1a5c3a]" />
                  AI 实时推理
                </span>
              ) : (
                <span className="mb-2 inline-flex items-center gap-1 rounded-full bg-[#fef3c7] px-2 py-0.5 font-mono text-[10px] font-semibold uppercase tracking-[0.08em] text-[#92400e]">
                  <span className="h-1.5 w-1.5 rounded-full bg-[#92400e]" />
                  模拟数据
                </span>
              )}
              <p className="mt-1 text-sm leading-6 text-[#40564d]">{aiReport.conclusion.summary}</p>
            </>
          ) : null}
        </StepCard>
